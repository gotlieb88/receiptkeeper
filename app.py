...
# Clear both persistent browser storage and the entire active Streamlit session.
if st.sidebar.button(
    "🗑️ Forget Key on This Browser",
    use_container_width=True,
):
    # Browser localStorage updates asynchronously. Give the component enough
    # time to persist the cleared value before rebuilding the Streamlit session.
    browser_storage.setItem(
        PRIVATE_KEY_STORAGE_NAME,
        "",
    )
    ...
    # Remove every server-side value associated with the current private space.
    st.session_state.clear()

    # Re-run from a clean state. The local-storage value is now empty, so the
    # application must return to the private-key entry screen.
    st.rerun()
