...
# Robust key loading - works on BOTH desktop (secrets.toml) and Render (environment variable)
OPENAI_API_KEY = (
    st.secrets.get("OPENAI_API_KEY")          # Local development
    or os.getenv("OPENAI_API_KEY")            # Render production
)

if not OPENAI_API_KEY:
    st.error("❌ OpenAI API key not found. Please set it in `.streamlit/secrets.toml` (local) or as OPENAI_API_KEY environment variable on Render.")
    st.stop()
