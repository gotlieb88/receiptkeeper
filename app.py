...
# ====================== PRIVATE KEY SYSTEM (Reliable Remember) ======================
...
with col1:
    if st.button("💾 Save / Remember Key"):
        if user_key:
...
with col2:
    if st.button("🔄 Load Saved Key"):
        st.markdown("""
            <script>
                const saved = localStorage.getItem('receiptkeeper_key');
                if (saved) {
        ...
        """, unsafe_allow_html=True)
        st.info("Check the field above and click Save / Remember Key if needed.")
...
# Optional: Forget button
if st.sidebar.button("🗑️ Forget Key"):
    st.markdown("""
        <script>
            localStorage.removeItem('receiptkeeper_key');
        </script>
    """, unsafe_allow_html=True)
    st.session_state.user_key = None
    st.rerun()
