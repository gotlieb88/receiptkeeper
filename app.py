...
user_key = st.sidebar.text_input(
    "Private Key",
    value=st.session_state.user_key or "",
    type="password",
    help="Choose something memorable. This separates your data from others on the hosted app."
)

if user_key and user_key != st.session_state.user_key:
    st.session_state.user_key = user_key
    st.rerun()

if not st.session_state.user_key:
    st.info("👆 Enter your private key in the sidebar to begin. Your data stays completely private and isolated.")
    st.stop()

st.sidebar.success(f"✅ Private space active: {st.session_state.user_key[:8]}...")

# ====================== PRIVATE KEY SYSTEM (with Remember) ======================
if "user_key" not in st.session_state:
    st.session_state.user_key = None
...
# Try to load from localStorage (JavaScript)
if st.session_state.user_key is None:
    st.markdown("""
        <script>
            if (localStorage.getItem('receiptkeeper_key')) {
                const key = localStorage.getItem('receiptkeeper_key');
...
user_key = st.sidebar.text_input(
    ...
    value=st.session_state.user_key or "",
    type="password",
    help="Choose something memorable. This separates your data from others."
)

remember = st.sidebar.checkbox("Remember my private key on this device", value=True)
...
if user_key and user_key != st.session_state.user_key:
    ...
    if remember:
        st.markdown(f"""
            <script>
                localStorage.setItem('receiptkeeper_key', '{user_key}');
            </script>
        """, unsafe_allow_html=True)
    st.rerun()
...
if os.path.exists("/data"):   # Render with persistent disk
    db_name = f"/data/receipts_{safe_hash}.db"
    os.makedirs("/data", exist_ok=True)
else:                         # Local development on your laptop
    ...
    if confirm == "RESET":
        if st.sidebar.button("✅ Confirm Permanent Deletion", type="primary"):
            try:
                ...
                # Optional: clear receipts folder (shared – use with caution)
                if os.path.exists("receipts"):
                    # shutil.rmtree("receipts")   # Uncomment only if you want to delete images too
